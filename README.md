...
I examined the `status_code` values to go beyond simply separating successful (`<400`) and failed (`>=400`) requests. This helped provide insight into how the system was being interacted with and what types of behaviors were present.

A high number of `200` responses indicates frequent interaction with the system, or possibly simulated success responses generated by the honeypot environment.

The presence of `404` errors suggests that attackers were actively scanning for non-existent directories or endpoints — a common tactic used in automated reconnaissance or brute-force tools.

Less frequent, but notable, `500` and `503` status codes could point to backend errors, exploitation attempts, or even signs of Denial-of-Service (DoS) testing using malformed or excessive requests.

These patterns reinforce the conclusion that the system was being targeted by automated or scripted tools, rather than random traffic, showing clear intent to discover or exploit vulnerabilities.
